...
impl Parse for DefineImageInput {
...
pub fn define_image_impl(input: TokenStream) -> TokenStream {
    let DefineImageInput {
        attrs,
        vis,
        name,
        lifetime,
        count,
        config,
    } = syn::parse_macro_input!(input as DefineImageInput);
    ...
    let expanded = quote! {
        ...
        impl crate::vulkan::resource_config::ImageMarker for #name {
            type Lifetime = crate::vulkan::resource_lifetime::#lifetime;
            const COUNT: usize = #count;
            const CONFIG: crate::vulkan::resource_config::ImageConfig = #config;
