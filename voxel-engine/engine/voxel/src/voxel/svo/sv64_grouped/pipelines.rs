...
const SV64_GROUPED_VOXEL_SHADER_PATH: &str =
    concat!(env!("VOXEL_SHADER_DIR"), "/voxel_sv64_grouped.spv");
const SV64_GROUPED_DYNAMIC_VOXEL_SHADER_PATH: &str =
    concat!(env!("VOXEL_SHADER_DIR"), "/dynamic_voxel_sv64_grouped.spv");
const SV64_GROUPED_VOXEL_DESTROY_SHADER_PATH: &str =
    concat!(env!("VOXEL_SHADER_DIR"), "/voxel_destroy_sv64_grouped.spv");

engine_macro::define_compute_pipeline! {
    pub struct VoxelPipeline;
    config = ComputePipelineConfig {
        shader_path: SV64_GROUPED_VOXEL_SHADER_PATH,
...
engine_macro::define_compute_pipeline! {
    ...
    config = ComputePipelineConfig {
        shader_path: SV64_GROUPED_DYNAMIC_VOXEL_SHADER_PATH,
...
engine_macro::define_compute_pipeline! {
    ...
    config = ComputePipelineConfig {
        shader_path: SV64_GROUPED_VOXEL_DESTROY_SHADER_PATH,
        descriptor_set_layouts: &[&COMPUTE_DESCRIPTOR_SET_LAYOUT_BINDINGS],
        push_constant_ranges: &VOXEL_PUSH_CONSTANT_RANGES,
    };
}
