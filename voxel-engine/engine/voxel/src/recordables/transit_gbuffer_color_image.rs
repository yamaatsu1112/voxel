use crate::builders::configs::image_configs::GBufferColorImage;
...
#[derive(Clone)]
pub struct TransitGBufferColorImage;

impl VkGraphicsRecordable for TransitGBufferColorImage {
    fn record(context: &mut VkGraphicsRecordContext) {
        let gbuffer_color_image = context
            .get_per_frame_image::<GBufferColorImage>(0)
            .expect("Failed to get G-Buffer color image");
