...
use crate::components::PauseUI;
use crate::scenes::{
    GameExitScene, GameScene, GameScenes, RenderGameScene, RenderMenuScene, RenderScenes,
};
use crate::systems::GameSceneCommand;

type ButtonDef = (u32, fn(&mut World), f32, f32, f32);

pub fn pause_setup_system(world: &mut World) {
    ...
    let buttons: [ButtonDef; 2] = [
        (0, resume_game_callback, 0.2, 0.6, 0.2),
        (1, return_to_menu_callback, 0.2, 0.4, 0.8),
    ];

    for (index, callback, r, g, b) in buttons.iter() {
        let button_y = center_y - (button_height + button_spacing)
...
fn resume_game_callback(world: &mut World) {
    let window = world.get_resource::<Window>().unwrap();
...
fn return_to_menu_callback(world: &mut World) {
    let mut scene_resource = world
        .get_resource_mut::<SceneResource<RenderScenes>>()
        .unwrap();
    scene_resource.set(RenderScenes::Menu(RenderMenuScene));
    world
        .get_resource::<GameChannel>()
        .unwrap()
        .write::<GameSceneCommand, _>(|value| {
            *value = Some(GameSceneCommand(GameScenes::GameExit(GameExitScene)));
        });
}

pub fn pause_cleanup_system(world: &mut World) {
    let query = world.query::<&PauseUI>();
