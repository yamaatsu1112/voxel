...
class SVO64Grouped {
  public:
    explicit SVO64Grouped(uint32_t max_depth = DEFAULT_MAX_DEPTH)
  ...
  private:
    ...
    static constexpr uint64_t kFullMask = ~uint64_t{0};
    static constexpr uint32_t kMaxSupportedDepth = 14;

    struct Mask64 {
        static constexpr uint32_t kFullWord = ~uint32_t{0};

        uint32_t lo = 0;
        uint32_t hi = 0;

        [[nodiscard]] bool has_bit(uint32_t bit_index) const {
            const uint32_t bit = uint32_t{1} << (bit_index & 31);
            return bit_index < 32 ? ((lo & bit) != 0) : ((hi & bit) != 0);
        }

        void set(uint32_t bit_index, bool value) {
            uint32_t& word = bit_index < 32 ? lo : hi;
            const uint32_t bit = uint32_t{1} << (bit_index & 31);
            if (value) {
                word |= bit;
            } else {
        ...
        [[nodiscard]] bool full() const {
            return lo == kFullWord && hi == kFullWord;
        }
    };
    ...
    struct Node {
        ...
        [[nodiscard]] bool has_child(uint32_t child_index) const {
            return child_mask.has_bit(child_index);
        }

        [[nodiscard]] bool is_filled(uint32_t child_index) const {
            return filled_mask.has_bit(child_index);
        }
        ...
        void set_child_mask(uint32_t child_index, bool value) {
            child_mask.set(child_index, value);
        }

        void set_filled(uint32_t child_index, bool value) {
            filled_mask.set(child_index, value);
        }
        ...
        [[nodiscard]] bool is_all_empty() const {
            return child_mask.empty() && filled_mask.empty();
        }

        [[nodiscard]] bool is_all_full() const {
            return child_mask.empty() && filled_mask.full();
        }
    };

    static_assert(sizeof(Mask64) == 8);
    ...
    [[nodiscard]] uint32_t allocate_node_block(uint32_t parent_index) {
        ...
        if (!free_node_block_indices_.empty()) {
            ...
            if (block_index == 0 || block_index > UINT32_MAX - kChildCount) {
                throw std::overflow_error("SVO64Grouped: node block index overflow");
            }
        ...
        if (block_index == 0 || block_index + kChildCount > nodes_.size()) {
            throw std::out_of_range(
                "SVO64Grouped: node block index out of bounds");
        }
    ...
    [[nodiscard]] uint32_t allocate_leaf_block(uint32_t parent_index) {
        ...
        if (!free_leaf_block_indices_.empty()) {
            ...
            if (block_index == 0 || block_index > UINT32_MAX - kChildCount) {
                throw std::overflow_error("SVO64Grouped: leaf block index overflow");
            }
        ...
        if (block_index == 0 || block_index + kChildCount > leaves_.size()) {
            throw std::out_of_range(
                "SVO64Grouped: leaf block index out of bounds");
        }
    ...
    void validate_coordinates(uint32_t x, uint32_t y, uint32_t z) const {
        if (x >= world_voxel_count() || y >= world_voxel_count() ||
            z >= world_voxel_count()) {
            throw std::out_of_range(
                "SVO64Grouped: voxel coordinates out of bounds");
        }
    }
    ...
    void validate_max_depth() const {
        if (max_depth_ == 0 || max_depth_ > kMaxSupportedDepth) {
            throw std::out_of_range(
                "SVO64Grouped: max_depth out of supported range");
        }
    }
