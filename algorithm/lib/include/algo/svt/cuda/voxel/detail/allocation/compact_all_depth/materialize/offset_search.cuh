...
__global__ void initialize_nodes_offset_search_kernel(
    ...
    const std::uint32_t owner =
        upper_bound_offset_rank(workspace.node_offsets, leaf_mask_capacity,
                                node_rank);
    if (owner >= leaf_mask_capacity)
        return;
    Initializer<InitMode, StartDepthMode>::initialize_node(
        svo, workspace, owner, node_rank);
}

template <class StartDepthMode, class Workspace>
__global__ void link_nodes_offset_search_kernel(
    ...
    const std::uint32_t depth = start_depth + (node_rank - node_base);
    link_one_node<StartDepthMode>(svo, leaf_masks, *leaf_count_ptr, workspace,
                                  owner, depth, node_rank);
}

template <class InitMode, class StartDepthMode, class Workspace>
__global__ void initialize_leaves_offset_search_kernel(
    DeviceGpuSvo svo, Workspace workspace, std::uint32_t leaf_mask_capacity) {
    ...
    const std::uint32_t owner =
        upper_bound_offset_rank(workspace.leaf_offsets, leaf_mask_capacity,
                                leaf_rank);
    if (owner >= leaf_mask_capacity)
        return;
...
template <class StartDepthMode, class Workspace>
__global__ void link_leaves_offset_search_kernel(
    DeviceGpuSvo svo, const LeafMask* leaf_masks,
    const std::uint32_t* leaf_count_ptr, Workspace workspace,
    ...
    if (owner >= leaf_mask_capacity)
        return;
    link_one_leaf<StartDepthMode>(svo, leaf_masks, *leaf_count_ptr, workspace,
                                  owner, leaf_rank);
}

template <class InitMode, class StartDepthMode>
struct Materializer<Nodewise<OffsetSearch>, InitMode, StartDepthMode> {
    ...
    static cudaError_t run(DeviceGpuSvo svo, const LeafMask* leaf_masks,
        ...
        cudaError_t status = resolve_materialized_node_request_count(
        ...
        if (node_request_count != 0u) {
            initialize_nodes_offset_search_kernel<InitMode, StartDepthMode>
        ...
        initialize_leaves_offset_search_kernel<InitMode, StartDepthMode>
        ...
        if (node_request_count != 0u) {
            link_nodes_offset_search_kernel<StartDepthMode>
        ...
        link_leaves_offset_search_kernel<StartDepthMode>
