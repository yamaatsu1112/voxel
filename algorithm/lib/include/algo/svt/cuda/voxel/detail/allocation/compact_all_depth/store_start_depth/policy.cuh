...
#include <algo/svt/cuda/voxel/detail/allocation/compact_all_depth/store_start_depth/workspace.cuh>
...
struct allocation_impl<
    CompactAllDepthAllocation<InitMode, StoreStartDepth, ScheduleMode>> {
    using Workspace =
        allocation::compact_all_depth::store_start_depth::Workspace;

    static std::size_t workspace_size(std::uint32_t count) {
        return allocation::compact_all_depth::store_start_depth::workspace_size(
            count,
            std::is_same_v<ScheduleMode, Nodewise<ExplicitRequests>>);
    }

    static Workspace create_workspace(void* workspace, std::uint32_t count) {
        return allocation::compact_all_depth::store_start_depth::
            create_workspace(
                workspace, count,
                std::is_same_v<ScheduleMode, Nodewise<ExplicitRequests>>);
    }
