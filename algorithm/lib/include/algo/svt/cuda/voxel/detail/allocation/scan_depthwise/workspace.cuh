...
struct Workspace {
    ...
    std::uint32_t* previous_request_emits;
    ...
    AllocationState* previous_allocation_state;
    AllocationState* current_allocation_state;
...
inline std::size_t workspace_size(std::uint32_t count) {
    ...
    offset += sizeof(AllocationState) * 2u;
...
inline Workspace create_workspace(
    ...
    reserve_array(workspace, count, offset, result.previous_request_emits);
    reserve_array(workspace, count, offset, result.current_request_emits);
    reserve_array(workspace, count, offset, result.unique_requests);
    reserve_array(workspace, 1, offset, result.unique_count);
    reserve_array(workspace, 1, offset, result.previous_allocation_state);
    reserve_array(workspace, 1, offset, result.current_allocation_state);
    result.request_keys = result.previous_request_emits;
    result.unique_offsets = result.current_request_emits;
    result.allocation_state = result.current_allocation_state;
