...
inline std::size_t collapse_workspace_size(std::uint32_t count) {
    return collapse::workspace_size(count);
}

inline collapse::Workspace create_collapse_workspace(void* workspace,
                                                     std::uint32_t count) {
    return collapse::create_workspace(workspace, count);
}
