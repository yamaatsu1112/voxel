...
inline constexpr uint32_t kDefaultPageBits = 15;

template <uint32_t PageBits = kDefaultPageBits>
class SVOForwardingImpl {
  public:
    explicit SVOForwardingImpl(uint32_t max_depth = DEFAULT_MAX_DEPTH)
    ...
    void set_voxel(uint32_t x, uint32_t y, uint32_t z, bool value) {
        ...
        for (uint32_t depth = 0; depth < max_depth_; ++depth) {
            ...
            // Child slot is just allocated under an initialized child group and is a
            // concrete node cell (not forwarding), so direct conversion is valid here.
  ...
  private:
    ...
    static constexpr uint32_t kGroupsPerPage = (1u << kPageBits) / kChildCount;
    static constexpr uint32_t kFreeGroupBitmapWords =
        (kGroupsPerPage + 63u) / 64u;
    static_assert(kPageBits >= 7 && kPageBits <= kDefaultPageBits,
    ...
    struct NodeCell {
        ...
        void set_child_mask_byte(uint8_t mask) {
            value = (value & ~kChildMaskMask) | uint32_t(mask);
        }
    ...
    // Utility conversion for internal cell-index usage.
    // Caller must ensure the target cell is already a resolved node cell
    // (i.e. not a forwarding cell).
    ...
    [[nodiscard]] std::pair<NodeIndex, NodeCell> resolve_node(CellIndex absolute_index) const {
        ...
        // `current` is a concrete node cell index after forwarding resolution.
    ...
    [[nodiscard]] GroupIndex group_index_from_cell_offset(CellOffset cell_offset) const {
        return GroupIndex{cell_offset.raw() / kChildCount};
    }

    [[nodiscard]] CellIndex group_start_index(PageIndex page_index,
                                              GroupIndex group_index) const {
        return CellIndex{
            page_index.raw() * cells_per_page() + group_index.raw() * kChildCount};
    }

    [[nodiscard]] NodeIndex node_index_at(CellIndex group_start, uint32_t slot_offset) const {
        // The slot in a child-node group is a node cell slot by construction.
        // This helper is used where the slot is known to be materialized as node data.
    ...
    [[nodiscard]] CellIndex allocate_group_in_page(PageIndex page_index) {
        ...
        if (page.free_group_count == 0) {
            throw std::overflow_error("SVOForwarding: page has no free child group");
        }
    ...
    struct RelocationTarget {
        CellIndex node_group_start_index;
        CellIndex child_group_start_index;
    };

    [[nodiscard]] RelocationTarget find_relocation_target() {
        for (uint32_t raw_page_index = 0; raw_page_index < pages_.size();
             ++raw_page_index) {
            const PageIndex page_index{raw_page_index};
            Page& page = pages_[page_index.raw()];
            if (page.free_group_count >= 2) {
                const GroupIndex node_group = take_free_group(page);
                const GroupIndex child_group = take_free_group(page);
                return RelocationTarget{
                    group_start_index(page_index, node_group),
                    group_start_index(page_index, child_group),
                };
            }
        }
        ...
        if (pages_.size() * std::size_t(cells_per_page()) >
            ...
            throw std::overflow_error("SVOForwarding: absolute index overflow");
        }
    ...
    [[nodiscard]] NodeIndex ensure_child_group(const NodeIndex node_index) {
        NodeCell node = get_node_cell(node_index);
        if (node.child_mask() != 0) {
            // if child group already exists
            return node_index;
        }
        ...
        // if the page is full
        const RelocationTarget target = find_relocation_target();
        const uint32_t slot_offset = node_index.raw() % kChildCount;
        const NodeIndex relocated_node_index =
            node_index_at(target.node_group_start_index, slot_offset);
        node.set_child_group_index(
            group_index_from_cell_offset(split_index(target.child_group_start_index).second));
        initialize_relocated_node_group(target.node_group_start_index,
                                        relocated_node_index,
                                        node);
        initialize_child_group(relocated_node_index, node);
        set_forwarding_cell(cell_index_from_node(node_index), relocated_node_index);
        return relocated_node_index;
    }

    void initialize_relocated_node_group(CellIndex group_start,
                                         NodeIndex relocated_node_index,
                                         const NodeCell& node) {
        for (uint32_t slot = 0; slot < kChildCount; ++slot) {
            init_node_cell(node_index_at(group_start, slot), NodeCell{});
        }
        set_node_cell(relocated_node_index, node);
    }
    ...
    void free_group(CellIndex group_start_absolute_index) {
        ...
        if ((cell_offset.raw() % kChildCount) != 0) {
            throw std::logic_error("SVOForwarding: group free requires group-aligned index");
        }
    ...
    void free_forward_chain_after_head(CellIndex head_index) {
        CellIndex current = head_index;
        while (is_forwarding_cell_value(get_cell_value(current))) {
            const CellIndex next{get_cell_value(current) & kForwardingIndexMask};
            if (current.raw() != head_index.raw()) {
                free_group(align_to_group_start(current));
            }
            current = next;
        }
        if (current.raw() != head_index.raw()) {
    ...
    void collapse_path(const std::vector<NodeIndex>& node_indices,
        ...
        for (std::size_t i = node_indices.size(); i-- > 1;) {
            ...
            free_forward_chain_after_head(child_cell_index);
    ...
    void validate_coordinates(uint32_t x, uint32_t y, uint32_t z) const {
        if (x >= world_voxel_count() || y >= world_voxel_count() ||
            ...
            throw std::out_of_range(
                "SVOForwarding: voxel coordinates out of bounds");
        }
    }
    ...
    void validate_max_depth() const {
        if (max_depth_ == 0 || max_depth_ > kMaxSupportedDepth) {
            throw std::out_of_range(
                "SVOForwarding: max_depth out of supported range");
        }
    }
    ...
    [[nodiscard]] std::pair<PageIndex, CellOffset> split_index(
        ...
        if (page_index.raw() >= pages_.size()) {
            throw std::out_of_range("SVOForwarding: absolute index out of bounds");
        }
        return {page_index, cell_offset};
    }

    [[nodiscard]] static CellIndex align_to_group_start(CellIndex absolute_index) {
        return CellIndex{absolute_index.raw() - (absolute_index.raw() % kChildCount)};
    }
    ...
    static void mark_group_used(Page& page, GroupIndex group_index) {
        ...
        if ((page.free_group_bitmap[word_index] & mask) == 0) {
            throw std::logic_error("SVOForwarding: allocating used child group");
        }
    ...
    static void release_group(Page& page, GroupIndex group_index) {
        ...
        const uint64_t mask = uint64_t{1} << bit_index;
        if ((page.free_group_bitmap[word_index] & mask) != 0) {
            throw std::logic_error("SVOForwarding: double free child group");
        }
        page.free_group_bitmap[word_index] |= mask;
        ++page.free_group_count;
    }
...
using SVOForwarding = SVOForwardingImpl<>;
