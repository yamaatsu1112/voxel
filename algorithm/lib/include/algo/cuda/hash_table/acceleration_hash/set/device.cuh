...
namespace acceleration_hash::set::detail {
...
__device__ inline bool slot_matches(Slab *slab, std::uint32_t slot,
  ...
  if (lane < ItemWords)
    word_equal = slab->items[slot][lane] == selected_word;
...
__device__ inline void acceleration_hash_set_find(Table table, bool &is_active,
                                                  const std::uint32_t *item,
                                                  std::uint32_t &ptr,
                                                  std::uint32_t &status) {
  static_assert(ItemWords > 0 && ItemWords <= 31,
                "Acceleration hash device API supports 1..31 words");
  ...
  while (work_queue != 0) {
    ...
    for (std::uint32_t visited = 0; !done && visited < max_chain_length;
      ...
      while (candidate_mask != 0) {
        ...
        const bool match =
            acceleration_hash::set::detail::slot_matches<ItemWords>(
...
acceleration_hash_set_insert_unique_unchecked(Table table, bool &is_active,
                                              const std::uint32_t *item,
                                              std::uint32_t &ptr,
                                              std::uint32_t &status) {
  static_assert(ItemWords > 0 && ItemWords <= 31,
    ...
    for (std::uint32_t attempts = 0; !done && attempts < max_attempts;
      ...
      if (empty_mask != 0) {
        ...
        if (old == kAccelerationHashEmpty) {
          if (lane < ItemWords)
            slab->items[slot][lane] = selected_word;
...
acceleration_hash_set_read_item(Table table, std::uint32_t ptr,
  ...
  for (std::uint32_t i = 0; i < ItemWords; ++i)
    out[i] = slab->items[slot][i];
  return true;
}

template <std::uint32_t ItemWords, class Allocator>
__device__ inline void DeviceAccelerationHashSet32<ItemWords, Allocator>::find(
    bool &is_active, const std::uint32_t *item,
    std::uint32_t &ptr, std::uint32_t &status) const {
  acceleration_hash_set_find<ItemWords>(*this, is_active, item, ptr, status);
}

template <std::uint32_t ItemWords, class Allocator>
__device__ inline void
DeviceAccelerationHashSet32<ItemWords, Allocator>::insert_unique_unchecked(
    bool &is_active, const std::uint32_t *item,
    std::uint32_t &ptr, std::uint32_t &status) {
  acceleration_hash_set_insert_unique_unchecked<ItemWords>(*this, is_active,
                                                           item, ptr, status);
}

template <std::uint32_t ItemWords, class Allocator>
__device__ inline bool
DeviceAccelerationHashSet32<ItemWords, Allocator>::read_item(
    std::uint32_t ptr, std::uint32_t (&out)[ItemWords]) const {
  return acceleration_hash_set_read_item<ItemWords>(*this, ptr, out);
}
