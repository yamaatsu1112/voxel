...
TEST(SvoForwardingPacked, DefaultsToEmpty) {
    algo::svt::SVOForwardingPacked svo;
    EXPECT_FALSE(svo.get_voxel(0, 0, 0));
    EXPECT_FALSE(svo.get_voxel(17, 9, 31));
}

TEST(SvoForwardingPacked, UsesConstructorMaxDepthForBounds) {
    algo::svt::SVOForwardingPacked svo(2);
...
TEST(SvoForwardingPacked, SetAndResetVoxel) {
    algo::svt::SVOForwardingPacked svo;
...
TEST(SvoForwardingPacked, CollapsesFullyFilledTerminalNodeBackIntoParentBit) {
    algo::svt::SVOForwardingPacked svo;
    ...
    EXPECT_LT(svo.node_count(), node_count_before + svo.max_depth() * 9u);
}

TEST(SvoForwardingPacked,
...
TEST(SvoForwardingPacked, ObservesForwardingAndAdditionalPagesAfterOverflow) {
    algo::svt::SVOForwardingPackedImpl<7> svo(5);
...
TEST(SvoForwardingPacked, ClearingForwardedTreeKeepsCountsSane) {
    algo::svt::SVOForwardingPackedImpl<7> svo(3);
